import React, { useRef } from 'react';
import { motion, useScroll, useTransform, MotionValue } from 'framer-motion';
import { ChevronDown, MapPin, MoveDown } from 'lucide-react';
import { IMAGES } from '../assets/images';
import { FAQ } from './FAQ';

const SCENES = [
  {
    id: 1,
    img: IMAGES.hero,
    content: (
      <>
        <motion.p className="text-gray-200 font-sans tracking-[0.4em] text-[10px] md:text-sm uppercase mb-4 md:mb-6 font-bold">
          1 de Agosto de 2026
        </motion.p>
        <h1 className="text-5xl md:text-9xl font-serif text-[#D8CFC4] leading-none tracking-tight mb-6 md:mb-8 drop-shadow-2xl">
          Paz <span className="text-[#F3EFE8] italic">&</span> David
        </h1>
        <p className="font-sans font-medium text-lg md:text-2xl text-wedding-50 tracking-wide text-[#D8CFC4] max-w-2xl mx-auto leading-relaxed">
          La Granja de Lozoya
        </p>
        <p className="font-serif italic text-lg md:text-xl text-gray-200 max-w-lg mx-auto leading-relaxed border-t border-white pt-6 mt-6">
          "Un día para celebrar lo que somos, con quien más queremos."
        </p>
      </>
    )
  },
  {
    id: 2,
    img: IMAGES.image1,
    content: (
      <>
        <h2 className="text-4xl md:text-8xl text-[#D8CFC4] font-serif text-wedding-50 italic mb-6 md:mb-8 drop-shadow-xl">
          ¡Gracias por acompañarnos!
        </h2>
        <div className="w-16 md:w-24 h-px bg-white mx-auto my-6 md:my-8"></div>
        <p className="font-serif italic text-lg md:text-2xl text-wedding-50 tracking-wide text-white/90 max-w-2xl mx-auto leading-relaxed">
          Está web ha sido creada con mucho cariño para que podáis encontrar toda la información sobre nuestro gran día de forma sencilla.
        </p>
      </>
    )
  },
  {
    id: 3,
    img: IMAGES.image4,
    content: (
      <>
        <h2 className="text-4xl md:text-8xl text-[#D8CFC4] font-serif italic mb-6 md:mb-8 drop-shadow-xl">
          Información Práctica
        </h2>
        <div className="w-16 md:w-24 h-px bg-white mx-auto my-6 md:my-8"></div>
      </>
    )
  },
  {
    id: 4,
    img: IMAGES.image3,
    content: (
      <div className="flex flex-col items-center justify-center gap-8">
        <div className="flex flex-col items-center justify-center gap-4 max-w-2xl px-4">
          <h2 className="text-3xl md:text-5xl text-[#D8CFC4] font-serif italic drop-shadow-xl">
            Cuándo & Dónde
          </h2>
          <p className="font-sans text-sm md:text-lg text-white/80 tracking-wide">
            Sábado, 1 de Agosto de 2026
          </p>
          <div className="w-12 md:w-16 h-px bg-white/40 mx-auto"></div>
        </div>
        <div className="relative z-30 w-full max-w-5xl px-4">
          <FAQ />
        </div>
      </div>
    )
  },
  {
    id: 5,
    img: IMAGES.image3,
    content: (
      <div className="flex flex-col items-center justify-center gap-8">
        <div className="flex flex-col items-center justify-center gap-4 max-w-2xl px-4">
          <h2 className="text-3xl md:text-5xl text-[#D8CFC4] font-serif italic drop-shadow-xl">
            Horarios
          </h2>
          <div className="w-12 md:w-16 h-px bg-white/40 mx-auto"></div>
        </div>
      </div>
    )
  }
];

const BackgroundLayer: React.FC<{
  img: string;
  index: number;
  range: [number, number];
  scrollYProgress: MotionValue<number>;
}> = ({ img, index, range, scrollYProgress }) => {
  const opacity = useTransform(
    scrollYProgress,
    index === 0
      ? [0, range[1] - 0.15, range[1]]
      : [range[0] - 0.1, range[0], range[1] - 0.1, range[1]],
    index === 0 ? [1, 1, 0] : [0, 1, 1, 0],
    { clamp: false }
  );
  const scale = useTransform(scrollYProgress, range, [1.05, 1], { clamp: false });

  return (
    <motion.div
      style={{ opacity, zIndex: index }}
      className="absolute inset-0 w-full h-full pointer-events-none will-change-transform"
    >
      <motion.div
        style={{ scale }}
        className="w-full h-full will-change-transform"
        initial={false}
      >
        <img
          src={img}
          alt="Background"
          className="absolute inset-0 w-full h-full object-cover"
          style={{
            contentVisibility: 'auto',
            backfaceVisibility: 'hidden'
          }}
          loading={index === 0 ? 'eager' : 'lazy'}
          decoding="async"
        />
        <div className="absolute inset-0 bg-black/50"></div>
      </motion.div>
    </motion.div>
  );
};

const TextLayer: React.FC<{
  children: React.ReactNode;
  index: number;
  range: [number, number];
  scrollYProgress: MotionValue<number>;
}> = ({ children, index, range, scrollYProgress }) => {
  const opacity = useTransform(
    scrollYProgress,
    index === 0
      ? [0, range[0] + 0.05, range[1] - 0.15, range[1]]
      : [range[0] - 0.05, range[0] + 0.05, range[1] - 0.15, range[1]],
    index === 0 ? [1, 1, 1, 0] : [0, 1, 1, 0],
    { clamp: false }
  );
  const y = useTransform(
    scrollYProgress,
    [range[0], range[0] + 0.2, range[1] - 0.2, range[1]],
    [30, 0, 0, -30],
    { clamp: false }
  );

  return (
    <motion.div
      style={{ opacity, y, pointerEvents: 'none' }}
      className="absolute inset-0 w-full h-full flex flex-col items-center justify-center text-center px-4 md:px-6 z-20 will-change-transform"
    >
      <div className="max-w-5xl">{children}</div>
    </motion.div>
  );
};

export const Hero: React.FC = () => {
  const containerRef = useRef<HTMLDivElement>(null);
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start start", "end end"]
  });

  return (
    <div ref={containerRef} className="relative h-[400vh] w-full">
      <div className="sticky top-0 h-screen w-full overflow-hidden bg-[#2F3E34]/70">
        {SCENES.map((scene, i) => {
          const step = 1 / SCENES.length;
          const start = i * step;
          const end = start + step;
          return (
            <React.Fragment key={scene.id}>
              <BackgroundLayer img={scene.img} index={i} range={[start, end]} scrollYProgress={scrollYProgress} />
              <TextLayer index={i} range={[start, start + step]} scrollYProgress={scrollYProgress}>
                {scene.content}
              </TextLayer>
            </React.Fragment>
          );
        })}
        <motion.div
          style={{ opacity: useTransform(scrollYProgress, [0, 0.01], [1, 0]) }}
          className="absolute bottom-10 left-1/2 -translate-x-1/2 z-50 text-white flex flex-col items-center gap-2 pointer-events-none"
        >
          <div className="animate-bounce mt-8 md:mt-12">
            <span className="text-[9px] md:text-[10px] uppercase tracking-[0.3em] font-sans font-bold text-gray-200">Desliza para comenzar</span>
            <MoveDown className=" text-gray-200 mx-auto" />
          </div>
        </motion.div>
      </div>
    </div>
  );
};